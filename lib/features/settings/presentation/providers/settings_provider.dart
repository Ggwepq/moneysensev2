...
class AppSettingsNotifier extends Notifier<AppSettings> {
  ...
  void toggleGoBackTimer(bool enabled) {
    if (enabled) {
      _update(
        state.copyWith(goBackTimerSeconds: _lastTimerSeconds.clamp(5, 60)),
      );
    } else {
  ...
  void setVisionProfile(VisionProfile profile) =>
      _update(state.copyWith(visionProfile: profile));

  void toggleTts(bool value) => _update(state.copyWith(ttsEnabled: value));
